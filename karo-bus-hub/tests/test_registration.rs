...
use karo_bus_common::{HUB_SOCKET_PATH_ENV, SERVICE_FILES_DIR};
...
async fn test_non_existing_service_name() {
    ...
    let service_dir =
        TempDir::new("karo_test_non_existing_service_name").expect("Failed to create tempdir");
...
async fn test_valid_registration() {
    ...
    let service_name = "com.karo.service.name";
    write_service_file(service_dir.path(), service_name, service_file_json).await;
